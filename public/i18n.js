...
export function t(value){
 ...
 if(result===undefined){
  result=key.replace(/^650 distinct projects and 1,658 occurrences[\s\S]*$/, '2026年9月10日从官方日程导入：9月11–13日共有650个独立项目、1,658条场次记录，包含单件作品、节目集合、重复场次与开放时段，并不代表1,658场独立演出。周五凌晨内容可能延续自周四夜间。“官方推荐”依官方标记；“精选推荐”为本指南选择。精选项目有中文背景介绍，其他项目保留简短官方英文原文及作者、主题信息。活动变更和实时名额请以官方链接为准。').replace(/^(Friday|Saturday|Sunday) (\d+) Sept$/,(m,day,n)=>`9月${n}日 ${days[day]}`)
   .replace(/^(FRIDAY|SATURDAY|SUNDAY) (\d+) SEPT$/,(m,day,n)=>`9月${n}日 ${days[day[0]+day.slice(1).toLowerCase()]}`)
   .replace(/^(\d+) projects?$/, '$1 个项目')
   ...
   .replace(/^(Save|Remove|Add|Visit start for|Visit duration in minutes for) (.+)$/,(m,verb,name)=>`${({Save:'收藏',Remove:'移除',Add:'添加','Visit start for':'参观开始时间：','Visit duration in minutes for':'参观时长（分钟）：'})[verb]} ${zh[name]||name}`)
  ...
  for(const en of ['EXHIBITION','PROJECT','COLLECTION','EXPERIENCE','CONCERT','PERFORMANCE','SCREENING','GUIDED TOUR','CONFERENCE','KEYNOTE','WORKSHOP','OPEN LAB','LECTURE & TALK','PANEL DISCUSSION','NETWORKING EVENT','MEET THE ARTIST'])result=result.replaceAll(en,zh[en]||en);
 }
...
function applyLanguage(){
 ...
 let node;while((node=walker.nextNode())){
  ...
  const next=t(originals.get(node));if(node.nodeValue!==next)node.nodeValue=next;
 }
 for(const el of document.querySelectorAll('[placeholder],[aria-label],[title],[alt]')){
  ...
  for(const attr of translatedAttributes)if(el.hasAttribute(attr)){
   ...
   const next=t(base[attr].source);base[attr].last=next;if(current!==next)el.setAttribute(attr,next);
  }
 }
